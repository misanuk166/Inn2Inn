...
export async function extractPois(cfg: RegionConfig): Promise<void> {
  ...
  log("pois", `${candidates.length} candidate POIs in region`);

  // For each POI, find every route in the region that passes within BUFFER_METERS.
  // We insert (route_id, osm_id) tuples (composite uniqueness on the table).
  const keptKeys = new Set<string>(); // `${route_id}|${osm_id}`
  let i = 0;
  for (const p of candidates) {
    ...
    if (i % 50 === 0 || i === candidates.length) {
      progress("pois", i, candidates.length);
    }
  }
