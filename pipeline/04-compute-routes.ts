...
//
// Concurrency: capped via PIPELINE_CONCURRENCY env var (default 10). Local
// OSRM tolerates 20+ comfortably; ORS paces itself internally so concurrency
// effectively saturates at the rate-limit anyway. Set to 1 if you want
// linear progress logs for debugging.
...
// Douglas-Peucker tolerance for stored polylines. Polyline geography is in
// degrees (EPSG:4326). 5e-5° ≈ 4–5 meters at California latitudes — well
// below visual resolution at any zoom level we render at. Cuts geometry
// storage 60–80% with no perceptible change.
const SIMPLIFY_TOLERANCE_DEG = 5e-5;
...
export async function computeRoutes(cfg: RegionConfig): Promise<number> {
  ...
  log("routes", `${hotels.length} lodgings to pair (concurrency=${concurrency})`);
  ...
  await Promise.all(
    [...candidatePairs].map((pairKey) =>
      limit(async () => {
        ...
        const samplePoints = sampleAlong(
          routeResult.geometry.coordinates as LngLat[],
          SAMPLES_PER_ROUTE
        );
        ...
        // WKT for ST_GeogFromText. Then simplify before storage.
        const lineWkt =
          "LINESTRING(" +
        ...
        try {
          const insertResult = await client.query<{ id: string }>(
            `insert into routes
             ...
             values ($1, $2, $3, $4, $5, $6, $7,
                     st_simplifypreservetopology(st_geogfromtext($8)::geometry, $10)::geography,
                     $9, 0, '{}'::jsonb, 'urban_road')
          ...
          log("routes", `insert error for ${aId}↔${bId}: ${(e as Error).message.slice(0, 100)}`);
        }
        tickProgress();
      })
    )
  ...
  function tickProgress() {
    completed++;
    if (completed % 25 === 0 || completed === candidatePairs.size) {
      progress("routes", completed, candidatePairs.size, `kept ${keptIds.size}`);
    }
  }
